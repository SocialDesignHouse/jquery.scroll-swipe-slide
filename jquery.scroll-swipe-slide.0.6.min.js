...
return true;}else{return e.settings[d];}}else{return false;}}else{return false;}}}c=a.extend({},a.fn.scrollSwipeSlide.default_settings,c||{});return this.each(function(){var g=a(this);
var f=jQuery.extend(true,{},c);var h=new b(f);h.scrollswipeslide=g;h.initialize();g.data("_scrollSwipeSlide",h);console.log(g.data("_scrollSwipeSlide"));
});};a.fn.scrollSwipeSlide.default_settings={use_scrollto:true,use_swipe:true,use_keypress:true,use_history:false,base_url:"",multi_dir:false,go_to_next_container:false,container:".project",container_before:"",container_after:"",slides:".slide",slides_before:"",slides_after:"",nav:".slide-nav",slideshow_class:".slideshow",easing:"easeInOutExpo",scroll_time:1000,scroll_lockout:20,width:"100%",height:"100%",skip_first:false,callback:""};
...
a(d).addClass("active");a(d).find(g.slides+":first-child").addClass("current");j+="</ul></nav>";h.scrollswipeslide.prepend(j);},slide_it:function(){var d=this;
var c=d.settings;if(d.direction){if(d.direction=="u"){if(a(c.slideshow_class).find(".active").prev(c.container).length>0){d.scrolling=true;d.go_to=a(c.slideshow_class).find(".active").prev(c.container);
d.slide_vertical();}else{d.scrolling=false;}}else{if(d.direction=="d"){if(a(c.slideshow_class).find(".active").next(c.container).length>0){d.scrolling=true;
d.go_to=a(c.slideshow_class).find(".active").next(c.container);d.slide_vertical();}else{d.scrolling=false;}}else{if(d.direction=="r"&&c.multi_dir){if(a(c.container+":eq("+d.container+")").find(".current").next(c.slides).length>0){d.scrolling=true;
d.go_to=a(c.container+":eq("+d.container+")").find(".current").next(c.slides);d.slide_horizontal();}else{if(c.go_to_next_container&&a(c.container+":eq("+d.container+")").next(c.container).length>0){d.go_to=a(c.container+":eq("+d.container+")").next(c.container);
d.direction="d";d.slide_vertical();}else{d.scrolling=false;}}}else{if(d.direction=="l"&&c.multi_dir){if(a(c.container+":eq("+d.container+")").find(".current").prev(c.slides).length>0){d.scrolling=true;
d.go_to=a(c.container+":eq("+d.container+")").find(".current").prev(c.slides);d.slide_horizontal();}else{if(c.go_to_next_container&&a(c.container+":eq("+d.container+")").prev(c.container).length>0){d.go_to=a(c.container+":eq("+d.container+")").prev(c.container);
d.direction="d";d.slide_vertical();}else{d.scrolling=false;}}}}}}}else{d.scrolling=false;}},slide_vertical:function(){var f=this;var e=f.settings;if(f.go_to){var d=f.go_to;
var c=d.index();f.container=c;if(e.container_before){e.container_before.call(this);}a(e.nav).find(".active").removeClass("active");a(e.nav).find(".slide-circle:eq("+c+")").addClass("active");
a(e.slideshow_class).stop(true,true).scrollTo(d,e.scroll_time,{easing:e.easing,onAfter:function(){scroll_timeout=setTimeout(function(){f.scrolling=false;
},e.scroll_lockout);if(a(e.container+":eq("+c+")").find(".current").length<1){a(e.container+":eq("+c+")").find(e.slides+":first").addClass("current");}}});
f.current=d;f.update_history();a(e.sldeshow_class).find(".active").find(".current").removeClass("current");if(!d.hasClass("active")){a(e.slideshow_class).find(".active").removeClass("active");
d.addClass("active");}if(e.container_after){e.container_after.call(this);}}},update_history:function(){var g=this;if(g.history){var f=g.current;var i=f.attr("id");
...
a(document).keyup(function(g){if(g.preventDefault){g.preventDefault();}if(c.use_keys){var f=g.which;if(!d.scrolling){d.direction="";if(f==40||f==34){d.direction="d";
}else{if(f==38||f==33){d.direction="u";}else{if(f==36||f==35){return false;}else{if(c.multi_dir&&f==37){d.direction="l";}else{if(c.multi_dir&&f==39){d.direction="r";
}}}}}d.slide_it();}}return false;});},enable_mousewheel:function(){var c=this;a("body").bind("mousewheel",function(f,g){if(f.preventDefault){f.preventDefault();
}if(!c.settings.skip_first||c.this_scroll>0){if(!c.scrolling){c.this_scroll++;if(c.this_scroll===0||c.last_scroll!=c.this_scroll){if(g>0){c.direction="u";
}else{if(g<0){c.direction="d";}}c.slide_it();}c.last_scroll=c.this_scroll;}}else{c.scrolling=true;var d=setTimeout(function(){c.scrolling=false;},750);
c.this_scroll++;}});},enable_swipe:function(){var c=this;settings=c.settings;if(settings.use_swipe){a("body").on("swipeup",function(d){if(d.preventDefault){d.preventDefault();
...
var c=a(".project:eq("+e.go_to+")");e.container=e.go_to;if(d.container_before){d.container_before.call(this);}c.addClass("active");c.find(d.slides+":first-child").addClass("current");
...
}if(e.slides_after){e.slides_after.call(this);}}},next_slide:function(){},prev_slide:function(){},next_container:function(){},prev_container:function(){},go_to_container:function(c){},go_to_slide:function(c){}};
})(jQuery);