...
}var c=20*a(d.nav).find(".slide-circle").length;var e=(f.win_height-c)/2;a(d.nav).css({margin:e+"px 0"});if(d.multi_dir){f.multi_dir_css();}},build_nav:function(){var h=this;
var g=h.settings;var j='<nav class="'+g.nav.substring(1)+'"><ul>';var c=a(g.slideshow_class).find(g.container).length;for(var e=0;e<c;e++){var f="";if(e===0){f=" active";
}j+='<li class="slide-circle'+f+'"><a href="javascript:void(0);"></a></li>';}var d=g.container+":first-child";a(d).addClass("active");a(d).find(g.slides+":first-child").addClass("current");
j+="</ul></nav>";h.scrollswipeslide.prepend(j);},update_history:function(){var g=this;if(g.history){var f=g.current;var i=f.attr("id");var h=f.data("title");
var c=settings.base_url+f.data("slug");var e=f.data("year");var d={id:i,title:h,year:e};History.pushState(d,h,c);}else{return false;}},check_url:function(){var f=this;
...
f.container=a(this).index();f.go_to=a(this).index();}f.switch_nav(f.go_to);return false;}});}else{if(!e.skip_first){f.current=a(e.container+":first-child");
f.update_history();}a(e.container+":first-child").addClass("active");return false;}},disable_scroll:function(){var d=this;var c=d.settings;a(window).scroll(function(f){if(!c.enable_scroll){if(f.preventDefault){f.preventDefault();
...
History.Adapter.bind(window,"statechange",function(){var f=History.getState();var h=document.URL;if(h.substr(-1)=="/"){h=h.substr(0,h.length-1);}var g=c.base_url.length;
