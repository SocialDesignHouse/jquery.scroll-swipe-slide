...
c.disable_scroll();a(window).resize(function(){c.set_css();});a(window).load(function(){c.set_css();});c.enable_mousewheel();c.bind_nav();if(c.settings.use_keypress){c.enable_keys();
...
var g=i.settings;if(i.history){var c=i.current;var h=c.data("slug");var f=c.attr("id");var k=c.data();var j;if(g.base_title){if(g.base_title_pos==="before"){j=g.base_title+g.base_title_sep+c.data("title");
...
var e={id:f,atts:k};History.pushState(e,j,d);}else{return false;}},check_url:function(){var f=this;var e=f.settings;var c=window.location.href;if(c!=e.base_url){f.skip_first=false;
var d=c.substring(e.base_url.length,window.location.href.length-1);a(e.container).each(function(){if(a(this).data("slug")==d){f.go_to=a(this).index();f.container=a(this).index();
f.switch_nav(f.go_to);return false;}});}else{if(!e.skip_first){f.current=a(e.container+":first-child");f.update_history();}a(e.container+":first-child").addClass("active");
return false;}},disable_scroll:function(){var d=this;var c=d.settings;a(window).scroll(function(f){if(!c.enable_scroll){if(f.preventDefault){f.preventDefault();
}return false;}});a(document).on("touchmove",function(f){if(!c.enable_scroll){if(f.preventDefault){f.preventDefault();}return false;}});},enable_keys:function(){var d=this;
var c=d.settings;a(document).keydown(function(g){if(g.preventDefault){g.preventDefault();}if(c.use_keypress){var f=g.which;if(!d.scrolling){d.direction="";
if(f==40||f==34){d.direction="d";}else{if(f==38||f==33){d.direction="u";}else{if(f==36||f==35){return false;}else{if(c.multi_dir&&f==37){d.direction="l";
...
d.go_to=f;d.current=a(c.container+":eq("+f+")");d.update_history();});},get_next:function(){var e=this;var d=e.settings;if(e.direction){var c=e.direction;
...
e.go_to=e.current.index();}else{e.scrolling=false;}}else{if(c==="d"){if(a(d.slideshow_class).find(".active").next(d.container).length>0){e.scrolling=true;
e.current=a(d.slideshow_class).find(".active").next(d.container);e.go_to=e.current.index();}else{e.scrolling=false;}}else{if(c==="l"&&d.multi_dir){if(a(d.container+":eq("+e.container+")").find(".current").prev(d.slides).length>0){e.scrolling=true;
e.go_to=a(d.container+":eq("+e.container+")").find(".current").prev(d.slides);e.slide_horizontal();}else{if(d.go_to_next_container&&a(d.container+":eq("+e.container+")").prev(d.container).length>0){e.current=a(d.container+":eq("+e.container+")").prev(d.container);
e.go_to=e.current.index();e.direction="u";}else{e.scrolling=false;}}}else{if(c==="r"&&d.multi_dir){if(a(d.container+":eq("+e.container+")").find(".current").next(d.slides).length>0){e.scrolling=true;
e.go_to=a(d.container+":eq("+e.container+")").find(".current").next(d.slides);e.slide_horizontal();}else{if(d.go_to_next_container&&a(d.container+":eq("+e.container+")").next(d.container).length>0){e.current=a(d.container+":eq("+e.container+")").next(d.container);
e.go_to=e.current.index();e.direction="d";}else{e.scrolling=false;}}}}}}e.update_history();}else{return false;}},switch_nav:function(e){var f=this;var c=f.settings;
var d=a(c.nav).find(".slide-circle:eq("+e+")");if(!d.hasClass("active")){d.parent().find(".active").removeClass("active");d.addClass("active");f.current=a(c.container+":eq("+e+")");
f.move_to();}else{return false;}},move_to:function(){var e=this;var d=e.settings;var c=a(d.container+":eq("+e.go_to+")");a(d.slideshow_class).find(".active").removeClass("active");
e.container=e.go_to;if(d.container_before){d.container_before.call(this);}c.addClass("active");c.find(d.slides+":first-child").addClass("current");a(d.slideshow_class).stop(true,true).scrollTo(c,d.scroll_time,{easing:d.easing,onAfter:function(){var f=setTimeout(function(){e.scrolling=false;
...
var e=h.substr(g);a(c.container).each(function(){if(a(this).data("slug")==e){var i=a(this).index();d.go_to=i;d.switch_nav(i);return false;}});});},multi_directional:function(){var d=this;
...
a(this).css({width:e+"px",position:"relative",left:"0px",top:"0px"});});},slide_horizontal:function(){var g=this;var e=g.settings;if(g.go_to){var c="";
...
}if(e.slides_after){e.slides_after.call(this);}g.direction="";}}};})(jQuery);