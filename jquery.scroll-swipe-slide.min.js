...
if(f.direction){f.scrolling=true;var c,d;if(f.direction=="u"){if(a(e.slideshow_class).find(".active").prev(e.container).length>0){f.go_to=a(e.slideshow_class).find(".active").prev(e.container);
f.slide_vertical();}else{f.scrolling=false;}}else{if(f.direction=="d"){if(a(e.slideshow_class).find(".active").next(e.container).length>0){f.go_to=a(e.slideshow_class).find(".active").next(e.container);
f.slide_vertical();}else{f.scrolling=false;}}else{if(f.direction=="r"){if(a(e.container+":eq("+f.container+")").find(".current").next(e.slides).length>0){f.go_to=a(e.container+":eq("+f.container+")").find(".current").next(e.slides);
f.slide_horizontal();}else{if(e.go_to_next_container&&a(e.container+":eq("+f.container+")").next(e.container).length>0){f.go_to=a(e.container+":eq("+f.container+")").next(e.container);
f.direction="d";f.slide_vertical();}else{f.scrolling=false;}}}else{if(f.direction=="l"){if(a(e.container+":eq("+f.container+")").find(".current").prev(e.slides).length>0){f.go_to=a(e.container+":eq("+f.container+")").find(".current").prev(e.slides);
f.slide_horizontal();}else{if(e.go_to_next_container&&a(e.container+":eq("+f.container+")").prev(e.container).length>0){f.go_to=a(e.container+":eq("+f.container+")").prev(e.container);
f.direction="d";f.slide_vertical();}else{f.scrolling=false;}}}}}}}},slide_vertical:function(){var e=this;var d=e.settings;if(e.go_to){var c=e.go_to;index=c.index();
e.container=index;if(d.container_before){d.container_before.call(this);}a(d.nav).find(".active").removeClass("active");a(d.nav).find(".slide-circle:eq("+index+")").addClass("active");
a(d.slideshow_class).stop(true,true).scrollTo(c,d.scroll_time,{easing:d.easing,onAfter:function(){e.this_scroll++;scroll_timeout=setTimeout(function(){e.scrolling=false;
...
});},enable_mousewheel:function(){var c=this;a("body").bind("mousewheel",function(f,g){if(f.preventDefault){f.preventDefault();}if(!c.settings.skip_first||c.this_scroll>0){if(!c.scrolling){if(c.this_scroll===0||c.last_scroll!=c.this_scroll){if(g>0){c.direction="u";
...
var d=e.settings;var c=a(".project:eq("+e.go_to+")");e.cotainer=e.go_to;if(e.slide===0){e.top=true;e.bottom=false;}else{if(e.slide==a(d.nav).find(".slide-circle").length-1){e.bottom=true;
e.top=false;}else{e.bottom=false;e.bottom=false;}}if(d.container_before){d.container_before.call(this);}c.addClass("active");c.find(d.slides+":first-child").addClass("current");
...
});d.bind_multi_dir_events();},multi_dir_css:function(){var d=this;var c=d.settings;a(c.slides).css({width:d.win_width+"px",height:d.win_height+"px","float":"left"});
a(c.container).each(function(){var f=a(this).data("slides");var e=d.win_width*f;a(this).css({width:e+"px",position:"relative",left:"0px",top:"0px"});});
},bind_multi_dir_events:function(){var d=this;var c=d.settings;if(c.enable_swipe){d.bind_left_right_swipe();}},bind_left_right_swipe:function(){a("body").on("swipeleft",function(c){if(c.preventDefault){c.preventDefault();
}$this.direction="l";$this.slide_it();return false;});a("body").on("swiperight",function(c){if(c.preventDefault){c.preventDefault();}$this.direction="r";
$this.slide_it();return false;});},slide_horizontal:function(){var g=this;var e=g.settings;if(g.go_to){var c="";var d=g.go_to;g.slide=d.index();if(e.slides_before){e.slides_before.call(this);
...
}}}else{c=0;}TweenMax.to(a(e.container+":eq("+g.container+")"),0.8,{css:{left:c+"px"},ease:Expo.easeInOut,onComplete:function(){scroll_timeout=setTimeout(function(){g.scrolling=false;
