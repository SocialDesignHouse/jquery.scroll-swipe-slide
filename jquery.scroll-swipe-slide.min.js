...
e.scrolling=false;e.this_scroll=0;e.last_scroll=0;e.container=0;e.slide=0;if(c.use_history&&c.base_url){var d=window.History;if(!d.enabled){e.history=false;
return false;}else{e.history=true;}}else{e.history=false;}e.check_url();e.bind_events();c.callback.call(this);},bind_events:function(){var c=this;c.disable_scroll();
a(window).resize(function(){c.set_css();});c.enable_mousewheel();c.bind_nav();if(c.settings.use_keypress){c.enable_keys();}if(c.settings.use_swipe){c.enable_swipe();
...
var g=f.win_width-20;a(d.container).css({width:g+"px"});}if(d.height=="100%"){a(d.slideshow_class).css({height:f.win_height+"px"});a(d.container).css({height:f.win_height+"px"});
}var c=20*a(d.nav).find(".slide-circle").length;var e=(f.win_height-c)/2;a(d.nav).css({margin:e+"px 0"});},build_nav:function(){var h=this;var g=h.settings;
var j='<nav class="'+g.nav.substring(1)+'"><ul>';var c=a(g.slideshow_class).find(g.container).length;for(var e=0;e<c;e++){var f="";if(e===0){f=" active";
}j+='<li class="slide-circle'+f+'"></li>';}var d=g.container+":first-child";a(g.slideshow_class).find(d).addClass("active");j+="</ul></nav>";h.scrollswipeslide.prepend(j);
},slide_it:function(){var f=this;var e=f.settings;if(f.direction){f.scrolling=true;var c,d;if(f.direction=="u"){if(a(e.slideshow_class).find(".active").prev(e.container).length>0){c=a(e.slideshow_class).find(".active").prev(e.container);
}else{return false;}}else{if(f.direction=="d"){if(a(e.slideshow_class).find(".active").next(e.container).length>0){c=a(e.slideshow_class).find(".active").next(e.container);
}else{return false;}}}if(c){d=c.index();f.container=d;e.container_before.call(this);a(e.nav).find(".active").removeClass("active");a(e.nav).find(".slide-circle:eq("+d+")").addClass("active");
...
}e.container_after.call(this);}}},update_history:function(){var g=this;if(g.history){var f=g.current;var i=f.attr("id");var h=f.data("title");var c="/"+f.data("slug");
var e=f.data("year");var d={id:i,title:h,year:e};History.pushState(d,h,c);}else{return false;}},check_url:function(){var f=this;var e=f.settings;var c=window.location.href;
...
e.top=false;}else{e.bottom=false;e.bottom=false;}}d.container_before.call(this);c.addClass("active");a(d.slideshow_class).stop(true,true).scrollTo(c,d.scroll_time,{easing:d.easing,onAfter:function(){var f=setTimeout(function(){e.scrolling=false;
},d.scroll_lockout);}});e.current=c;e.update_history();d.container_after.call(this);},bind_statechange:function(){History.Adapter.bind(window,"statechange",function(){var c=History.getState();
});}};})(jQuery);